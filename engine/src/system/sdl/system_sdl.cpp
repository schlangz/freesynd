...
void SystemSDL::initialize(bool fullscreen, int windowWidth, int windowHeight) {
    ...
    pWindow_ = createWindow(fullscreen, windowWidth, windowHeight);

    if (pWindow_ == nullptr) {
        throw InitializationFailedException(std::format("Critical error, SDL could not be initialized! SDL Error :  {}", SDL_GetError()));
    }

    // Confine the mouse cursor to the game window by default (Ctrl+Shift
    // temporarily releases it, e.g. to alt-tab or use a second monitor).
    SDL_SetWindowGrab(pWindow_, SDL_TRUE);

    // Force a modern backend. On Windows, SDL's default accelerated driver can
    // be Direct3D9, whose device-loss-on-reset model wipes the contents of any
    // SDL_TEXTUREACCESS_TARGET texture (like the menu background cache) on a
    // device reset - which a window resize can trigger. Direct3D11 doesn't
...
SDL_Window * SystemSDL::createWindow(bool fullscreen, int windowWidth, int windowHeight) {
...
bool SystemSDL::pumpEvents(FS_Event &evtOut) {
    ...
    // Some SDL events (window events, and a couple of modifier-key-up cases
    // below) don't produce a reportable FS_Event. Keep draining the queue
    // past those instead of returning early, so a burst of many queued
    // events (e.g. the many SDL_WINDOWEVENT_SIZE_CHANGED events generated by
    // a single interactive window resize drag) all get processed within one
    // call instead of trickling in one per frame, which left gameWidth_/
    // gameHeight_/the offscreen game texture settling on a stale
    // intermediate size for several frames.
    while (evtOut.type == EVT_NONE && SDL_PollEvent(&evtIn)) {
        switch (evtIn.type) {
        case SDL_QUIT:
            evtOut.quit.type = EVT_QUIT;
            break;
        case SDL_WINDOWEVENT:
            if (evtIn.window.event == SDL_WINDOWEVENT_SIZE_CHANGED
                || evtIn.window.event == SDL_WINDOWEVENT_RESIZED) {
                // Recompute the letterbox viewport for the new window size, and
                // recreate the offscreen game texture if its dimensions changed
                // (only happens in native-aspect/gameplay mode, where the game
                // texture is sized to fill the window at an integer scale).
                int oldGameWidth = gameWidth_;
                int oldGameHeight = gameHeight_;
                calculateGameViewport();
                if (gameWidth_ != oldGameWidth || gameHeight_ != oldGameHeight) {
            ...
            if (evtIn.window.event == SDL_WINDOWEVENT_RESIZED) {
                // Persist the new size to user.conf so it's restored on next launch.
                // Skipped in fullscreen since data1/data2 would then be the monitor
                // resolution, not a real windowed size chosen by the user.
                bool isFullscreen = (SDL_GetWindowFlags(pWindow_) &
                    (SDL_WINDOW_FULLSCREEN | SDL_WINDOW_FULLSCREEN_DESKTOP)) != 0;
                if (!isFullscreen) {
                    g_Ctx.updateWindowSize(evtIn.window.data1, evtIn.window.data2);
                }
            }
            break;
        case SDL_TEXTINPUT:
        ...
        case SDL_KEYDOWN:
            ...
            switch(evtIn.key.keysym.sym) {
                case SDLK_RSHIFT:
                    ...
                    // Ctrl+Shift toggles mouse grab; only on the actual keydown
                    // (not OS key-repeat) so holding both doesn't retoggle.
                    if (!evtIn.key.repeat && (keyModState_ & KMD_CTRL)) {
                        toggleMouseGrab();
                    }
                    break;
                case SDLK_LSHIFT:
                ...
                case SDLK_RCTRL:
                    ...
                    if (!evtIn.key.repeat && (keyModState_ & KMD_SHIFT)) {
                        toggleMouseGrab();
                    }
                    break;
                case SDLK_LCTRL:
                ...
                case SDLK_LALT:
                    ...
                    break;
                case SDLK_RETURN:
                    if (keyModState_ & KMD_ALT) {
                        // Alt+Enter toggles fullscreen; swallow it rather than
                        // passing it through as a regular Enter keypress.
                        toggleFullscreen();
                    } else {
...
void SystemSDL::toggleFullscreen() {
    bool isFullscreen = (SDL_GetWindowFlags(pWindow_) &
        (SDL_WINDOW_FULLSCREEN | SDL_WINDOW_FULLSCREEN_DESKTOP)) != 0;
    SDL_SetWindowFullscreen(pWindow_, isFullscreen ? 0 : SDL_WINDOW_FULLSCREEN_DESKTOP);
    // The viewport/game texture are recomputed by the SDL_WINDOWEVENT_SIZE_CHANGED
    // event this generates (handled in pumpEvents()); SDL also remembers/restores
    // the windowed size automatically across the fullscreen<->windowed transition.
    g_Ctx.updateFullscreen(!isFullscreen);
}

void SystemSDL::toggleMouseGrab() {
    SDL_SetWindowGrab(pWindow_, SDL_GetWindowGrab(pWindow_) ? SDL_FALSE : SDL_TRUE);
}
